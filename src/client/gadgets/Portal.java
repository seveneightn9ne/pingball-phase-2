 ...
 * it is teleported with the same velocity to a specified portal on a specified
 * board. If the board or the portal do not exist, then the ball exits the
 * portal.
 * 
 */
public class Portal implements Gadget {
	...
	/**
	 * Creates a portal on the specified board linked to the specified other
	 * portal.
	 * 
	 * @param name
	 *            Name of this board
	 ...
	 */
	public Portal(String name, int xPos, int yPos, String otherBoard,
			String otherPortal) {
		this.name = name;
		...
		portalShape = new Ellipse2D.Double(xPos * Constants.SCALE
				+ Constants.SCALE, yPos * Constants.SCALE + Constants.SCALE,
	...
	public boolean hit(Ball ball, Board board) {
		...
		if (otherBoard == null) {
			...
			ball.putInBoardRep(board, true);
			serverHandler.send(new TeleportOutMessage(ball.getVelocity(), board
					.getName(), name, otherBoard, otherPortal));
			return false;
		}
		return true;
	}
	...
	/**
	 * Gets the center of the circle that represents this portal.
	 * 
