 ...
 * 
 * Thread Safety Argument: - board and gadgets are confined to the main thread.
 * - incomingMessages is a threadsafe datatype. - serverHandler is a separate
 * thread that only shares data via incomingMessages.
 * 
 * Rep Invariant: - socket and serverHandler must either both be null or both be
 * non-null.
 */
public class PingballClient {
	 ...
	 * 
	 * @param board
	 *            the board to start the client with
	 * @param socket
	 *            socket to communicate with the server if socket is null,
	 *            server communication is disabled.
	 * @throws IOException
		...
		while (true) {
			...
			while (!incomingMessages.isEmpty()) {
				...
				if (message instanceof BallInMessage) {
					// The sending board is responsible for making ballPos on
					// the correct side of the receiving board.
	...
	private void checkRep() {
		if ((socket == null) != (serverHandler == null)) {
			throw new RepInvariantException(
					"socket and serverHandler must have the same nullness");
		}
	}
	...
	public static void main(String[] args) {
		...
		final Board board = Parser.makeBoard(new File(boardFilePath));
