...
/**
 * 
 * Pingball client creates a game board from a text file, connects to a pingball
...
public class PingballClient {
	...
	/**
	 * Create a Pingball client (This should only be called via
	 * PingballClient.main)
	 */
	...
	private void checkRep() {
		if ((board == null) && (serverHandler != null || boardPath != null)) {
			throw new RepInvariantException(
					"serverHandler and boardPath must be null if board is");
		}
	 ...
	 * 
	 *            HOST is an optional hostname or IP address of the server to
	 *            connect to. If no HOST is provided, then the client starts in
	 *            single-machine play mode.
	 * 
	 ...
	 * 
	 *            FILE is an optional argument specifying a file pathname of the
	 *            Pingball board that this client should run.
	 * 
	 */
	public static void main(String[] args) {
		...
		if (hostname != null) {
			try {
				...
				System.err
						.println("Tried to connect to server without a valid board");
			}
		}
		client.startClient();
	...
	/**
	 * Change the board to the one at path This will disconnect the client from
	 * the server
	 * 
	 * @param path
	 *            the file to read the board from. Must be a valid .pb board
	 *            file.
	 */
	...
	/**
	 * Connect to the server at hostname on the given port. If the client is
	 * already connected to a server, that connection will be ended.
	 * 
	 * @param hostname
	 ...
	 * @throws IOException
	 *             if the connection cannot be established. In this case, any
	 *             existing server connection will still be terminated.
	 */
	protected void connectToServer(String hostname, int port)
			throws IOException {

		if (board == null) {
			throw new RuntimeException(
					"Board must be set before connecting to server.");
		}
	...
	protected void disconnectFromServer() {
		...
		if (board != null)
			board.setServerHandler(null);
	}

	/**
	 * Stops the board from updating or from handling server messages TODO:
	 * should we disconnect from the server? or what?
	 */
	...
	/**
	 * Restart the client with the same board as is currently set. Requires that
	 * the client currently has a board.
	 */
